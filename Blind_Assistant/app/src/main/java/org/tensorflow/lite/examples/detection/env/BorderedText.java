...
public class BorderedText {
  ...
  public void drawText(
    ...
    String message = "";
    float d = DetectorActivity.d;
    TextToSpeech tts = DetectorActivity.tts;
    if (!prevObj.equalsIgnoreCase(text)) {
      if (d < 1) {
        message = String.format(text + " detected at %.2f feet", d);
      } else {
        message = text + " detected at " + (int) d + " feet";
      }

      int previewWidth = DetectorActivity.previewWidth;
      int rxpos = (int) (previewWidth/10.0*3);
      int lxpos = (int) (previewWidth/10.0*6);
      if (posX<rxpos){
